...
export default function SwipeCard({ track, onSwipe, audioControls }) {
  ...
  // Registered in an effect rather than mutated during render, and withdrawn on
  // unmount. Left behind, this handle keeps pointing at a card that no longer
  // exists, and the buttons animate controls nobody is watching — so the swipe
  // silently never completes.
  useEffect(() => {
    if (!audioControls) return;
    audioControls.triggerSwipe = triggerSwipe;
    return () => {
      if (audioControls.triggerSwipe === triggerSwipe) delete audioControls.triggerSwipe;
    };
  });
